...
FROM      (
  ...
  FROM      (
    ...
    public function getUserGerenciaSql2($country ='', $noemple='',$perfil){
      switch ($perfil){
      case "GC":
        $includeperfil = "'GC','GR','RG','CO','GE','GR','GA','GS','GD'";
        $includeNoCia = " ";
        break;
      case "RG":
        $includeperfil = "'GR','CO','GE','RG',GR','GA','GS','GD'";
        $includeNoCia = "no_cia = ".$country." and ";
        break;
      case "CO":
        $includeperfil = "'GR','GE','CO','GR','GA','GS','GD'";
        $includeNoCia = "no_cia = ".$country." and ";
        break;
      case "GR":
        $includeperfil = "'GR','GA','GS','GD','GR','GA','GS','GD'";
        $includeNoCia = "no_cia = ".$country." and ";
        break;
      case "GS":
        $includeperfil = "'GR','GA','GS','GD'";
        $includeNoCia = "no_cia = ".$country." and ";
        break;
      case "GG":
        $includeperfil = "'GR','GA','GS','GD";
      ...
      $query ="";
      $query .="SELECT u.id, lev.no_cia,  lev.no_emple,lev.nombre,lev.apellido,lev.no_rest, r.perfil";
      $query .=" from M_USER u";
      $query .=" inner JOIN LEA_EMPLEADO_MV LEV";
      $query .=" on u.idnumber = lev.no_emple and u.institution = lev.no_cia";
      $query .=" inner join mv_lea_rest_gerencia r";
      $query .=" on r.no_emple = lev.no_emple and r.no_cia = lev.no_cia";
      $query .=" WHERE lev.status_emp = 'A' and r.perfil in ($includeperfil)";
      $query .=" and lev.NO_REST IN (";
      $query .="     select r.no_rest from MV_LEA_REST_GERENCIA r";
      $query .="     where $includeNoCia no_emple='$noemple')";
      // echo $query;
      global $DB;
      return $DB->get_records_sql($query);    
  }

  public function getProfileData($nocia,$noemple){
    $query =" ";
    $query.="SELECT ";
    $query.="   DISTINCT mlrg.PERFIL ";
    $query.=" FROM ";
    $query.="   MV_LEA_REST_GERENCIA mlrg ";
    $query.=" WHERE ";
    $query.="   MLRG.NO_CIA = $nocia AND mlrg.NO_EMPLE ='$noemple' AND mlrg.PERFIL <>'OF'";
    global $DB;
    return $DB->get_records_sql($query);
  }
