...
class userModel
{
  ...
  private function getProfesoresPorpuestoprofesorsin()
  {
    ...
    $sql = 'SELECT u.id, u.username, e.NOMBRE, e.APELLIDO
      FROM {user} u
	       INNER JOIN LEA_EMPLEADO_MV e ON e.NO_CIA=u.institution AND e.NO_EMPLE=u.idnumber
        WHERE u.deleted = 0 AND e.COD_PUESTO IN (00011078,00021378,60000055,83,50,00011900,00011901,00011079,00011080,00011081,00024426,00011082,00015791,60000056,00015363,60000095,4,5,6,7,8,9,40,42,1,2,57,59,63,58,64,85)';
